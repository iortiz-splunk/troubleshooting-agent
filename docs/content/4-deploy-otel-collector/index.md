...
description: "Uninstall any existing collector and deploy the Splunk Distribution of the OpenTelemetry Collector on your Linux EC2 instance."
weight: 4
navTitle: "Deploy the OpenTelemetry Collector"
duration: "10 minutes"
---

## Uninstall the OpenTelemetry Collector

Our EC2 instance may already have an older version of the Splunk Distribution of the OpenTelemetry Collector installed. Before proceeding further, let's uninstall it using the following command:
...
We can do this by downloading the collector binary using `curl`, and then running it with specific arguments that tell the collector which realm to report data into, which access token to use, and which deployment environment to report into.

{{< notice title="About deployment environments" style="tip" >}}
...
--deployment-environment sre-agent-wkshp-$INSTANCE \
-- $ACCESS_TOKEN
```
...
Refer to [Install the Collector for Linux with the installer script](https://docs.splunk.com/observability/en/gdi/opentelemetry/collector-linux/install-linux.html#otel-install-linux) for further details on how to install the collector.
