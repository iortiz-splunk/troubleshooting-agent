...
from workshop_shared.tools.stubs import splunk_cloud_mcp, splunk_mcp, splunk_o11y
...
class McpSessionManager:
    ...
    # -----------------------------------------------------------------------
    # Context manager entry — connect each enabled Splunk MCP integration.
    # -----------------------------------------------------------------------
    ...
    async def _add_o11y(self) -> None:
        ...
        if not tools:
            msg = (
                ...
                "with: troubleshooting-agent mcp-doctor"
            )
    ...
    async def _add_splunk_cloud_mcp(self) -> None:
        ...
        if not tools:
            msg = (
                ...
                "and SPLUNK_CLOUD_MCP_TENANT with: troubleshooting-agent mcp-doctor"
            )
            raise RuntimeError(msg)
        self._langchain_tools.extend(tools)
    ...
    # -----------------------------------------------------------------------
    # Low-level session open — stdio transport + MCP initialize handshake.
    # -----------------------------------------------------------------------
